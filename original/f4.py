"""robot_face.py – threaded face renderer for a 240×320 ILI9341 on Raspberry Pi.
Auto‑initialises hardware and draws animated eyes with iris + pupil.

Changes in this revision
-----------------------
* **Fixed Image size mismatch** – always create a 240×320 buffer regardless of rotation.
* **Rotation still applied** (`rotation=90`) so the face is upright on breadboards.
* Added `SCREEN_W` / `SCREEN_H` constants; logic now uses those instead of `display.width`.
"""
...
# ---- physical screen dims never change ----
    ...
    def _loop(self):
        ...
        while self._running:
            ...
            for idx,(cx,cy) in enumerate(((w//2-eye_off,eye_y),(w//2+eye_off,eye_y))):
                eye_side = "left" if idx==0 else "right"
                if blinking and (blink_eye in ("both", eye_side)):
    ...
    try:
        while True:
            random.choice([
                ...
                lambda: face.blink(eye=random.choice(["both","left","right"])),
            ])()
