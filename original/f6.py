"""robot_face.py – threaded face renderer for 2.2″ 240×320 ILI9341

 * No longer relies on `rotation=` parameter (not supported by `adafruit_rgb_display`).
 * Draws into a 240×320 buffer and rotates **once** just before pushing to hardware.
 * Blink logic, iris, pupil, expressions unchanged.
"""
from __future__ import annotations

import queue, random, threading, time, pwmio, board
from typing import Tuple
from PIL import Image, ImageDraw

SCREEN_W, SCREEN_H = 240, 320          # physical pixels
ROTATE_CW = True                       # rotate image 90° CW before display
...
class RobotFace:
    ...
    def _loop(self):
        ...
        while self._running:
            ...
            if ROTATE_CW:
                self.display.image(img.rotate(270))  # 90° CW
            else:
